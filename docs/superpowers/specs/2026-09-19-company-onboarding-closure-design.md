...
状态：**已于 2026-09-25 完成第一优先级实现；后续数据补全事项继续单独推进。**
...
实施结果：刷新仍按财务、分部、治理、行情四个独立事务顺序执行，但页面新增模块级等待、进行中、成功、失败状态和绿色完成进度条。每个模块返回后立即原地重载公司数据；成功但内容未变化时明确显示“检查成功，暂无更新”。进度卡同时展示数据日期和按浏览器本地时区转换的本次检查时间，顶部来源信息继续展示抓取时间。部分失败保留已成功模块和旧完整快照，失败模块可单独重试；单模块网络级失败不会阻断后续模块，财务/行情变化也会立即触发估值复核。页面重新读取可选行情或新鲜度失败时保留上次完整快照并提示，不会清空成“未同步”。切换公司时清空旧进度并继续用请求代次隔离迟到响应。

验收覆盖：顺序及请求边界、实时进度、无更新语义、模块原地重载、部分失败与单模块重试、网络级失败、估值重算/复核、刷新前迟到估值响应、公司切换迟到响应，以及桌面/移动端无横向溢出。实现与验证记录同步追加到 `docs/reviews/2026-09-11-company-onboarding-progress.md`。
